...
windmill.xhr = new function() {
    ...
    this.actionHandler = function(str) {
        ...
        if (_this.dataObj.error) {
            ...
            if (_this.action.method != 'defer') {
              ...
              windmill.stat("Ready, Waiting for tutorials...");
            
              if(windmill.xhr.tempMethod=='editor'){  //if we completed the loading and the last microstep loaded is 'editor' set its content
            	  if(tinyMCE.getInstanceById(windmill.xhr.tempActId)){
                      tinyMCE.get(windmill.xhr.tempActId).setContent(windmill.xhr.tempHtml); 
            	  }
            	  windmill.xhr.tempMethod='click'; //set tempMethod to the default value
              }
    ...
    this.createActionFromSuite = function(suiteName, actionObj) {
       ...
       /* if (suiteName == null) {
        ...
        var state = {method: actionObj.method, params: actionObj.params, action: action};
        var strIn=windmill.ui.remote.getInst(state);
        $(action.id+'inst').innerHTML='<br/>'+strIn;
        	
        this.tempMethod=actionObj.method;
    	if(actionObj.method=='editor'){
    		this.tempActId=action.id+'option';
    		this.tempHtml=actionObj.params['editor'];
    		tinyMCE.execCommand('mceAddControl', false, action.id+'option');
    	}
        
        if(windmill.ui.remote.simple==true){
