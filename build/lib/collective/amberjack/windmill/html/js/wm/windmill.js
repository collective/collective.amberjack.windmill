...
var windmill = new function() {
    ...
    //How long xhr waits in seconds before calling the timeout function
    this.xhrTimeout = 180;

    this.serviceDelay = 0;
    this.serviceDelayRunning = 40;  //changed to 40 for allow the loading of the text into tiny editor
    ...
    this.prompts = true;
    //what to answer by default
    this.promptAnswer = "collective.amberjack.windmill is great!";
    //storage array for confirm strings
    this.promptStore = [];
           		...
           		if(allSuite.length>0){
           			...
           			if(lastSuite.hasChildNodes()){
           				...
           				if($(lastSuite.childNodes[lastAct].id+'method')){
           					method = $(lastSuite.childNodes[lastAct].id+'method').value;
        	   
           				}
           			}
           		 }
        	  }
        	  if (method != "waits.forPageLoad" && method!=null){ //when new page is loaded, add microstep with 'waits.forPageLoad' method and next add a new step
